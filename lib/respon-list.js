...
function saveResponList(_db, dbPath = './database/list-message.json') {
    fs.writeFileSync(dbPath, JSON.stringify(_db, null, 5))
}

function addResponList(groupID, key, response, isImage, image_url, _db, dbPath) {
    var obj_add = {
        id: groupID,
        key: key,
        response: response,
...
function delResponList(groupID, key, _db, dbPath) {
    let position = null
    Object.keys(_db).forEach((x) => {
        if (_db[x].id === groupID && _db[x].key === key) {
            position = x
...
function updateResponList(groupID, key, response, isImage, image_url, _db, dbPath) {
    let position = null
    Object.keys(_db).forEach((x) => {
        if (_db[x].id === groupID && _db[x].key === key) {
            position = x
