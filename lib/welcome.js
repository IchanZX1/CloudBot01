...
function normalizeParticipantJid(participant) {
  const rawJid = typeof participant === 'string'
    ? participant
    : (participant && (participant.phoneNumber || participant.jid || participant.id));
...
function applyGroupPlaceholders(text, userJid, groupName, groupDesc) {
  const normalizedUserJid = normalizeParticipantJid(userJid);
  return String(text || '')
    .replace(/@user/gi, `@${normalizedUserJid.split('@')[0]}`)
...
function buildWelcomeCanvasUrl(design, data) {
  ...
  const defaultBackground = 'https://i.pinimg.com/736x/b6/c2/99/b6c299ec4f1e221ea7f8dbe4e43aefe4.jpg';
  const sunsetBackground = 'https://i.pinimg.com/1200x/cd/13/f4/cd13f40c7bba94a8c56315f222538e77.jpg';
  ...
  if (selected === 'design3') {
    params.set('avatar', data.avatar);
    params.set('background', defaultBackground);
    params.set('description', `Heya! Welcome New Member!`);
...
module.exports.welcome = async (iswel, isleft, NanoBotz, anu) => {
  try {
    ...
    const participants = (Array.isArray(anu.participants) ? anu.participants : [])
